...
const MemberList = function(props) {
  ...
  const setMemberRole = (isChecked, index) => {
    ...
    if (isChecked) {
      boardMembers[index].role = constants.ROLES.ADMIN;
    } else {
      boardMembers[index].role = constants.ROLES.USER;
    }
        ...
        <Table>
          ...
          <TableBody>
            {members.map((row, index) => {
              if (row.role === constants.ROLES.ADMIN) {
                row.switch = true;
              } else {
                row.switch = false;
              }
              return (
                <TableRow key={index}>
                  ...
                  <TableCell align="center">
                    {row.role !== constants.ROLES.SUPER_ADMIN && (
                      <Switch
                        checked={row.switch}
                        onChange={event => {
